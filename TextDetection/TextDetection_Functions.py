    ...
    def restore_rectangle(orig, geomt):
        ...
        if orig_1.shape[0] > 0:
            ...
            p = np.array([-TRDL_1[:, 1] - TRDL_1[:, 3], -TRDL_1[:, 0] - TRDL_1[:, 2], np.zeros(TRDL_1.shape[0]), -TRDL_1[:, 0] - TRDL_1[:, 2],
                          np.zeros(TRDL_1.shape[0]), np.zeros(TRDL_1.shape[0]), -TRDL_1[:, 1] - TRDL_1[:, 3], np.zeros(TRDL_1.shape[0]),
                          -TRDL_1[:, 1], -TRDL_1[:, 2]])
            p = p.transpose((1, 0)).reshape((-1, 5, 2))  #-> N*5*2
...
####################################################################################################
###################### FUNCTION 6: load_non_augmentation_images ####################################
####################################################################################################
...
def load_non_augmentation_images(imgs_and_anns_paths, input_size):
...
####################################################################################################
###################### FUNCTION 7: load_augmentation_images ##########################################
####################################################################################################
# DESCRIPTION: It is responsible for loading the images and paired annotations in the absolute "path."
# Later, it must correct the annotations and generate the maps of the image, to group them in
...
# and "crop_area".
def load_augmentation_images(imgs_and_anns_paths, input_size):
...
def loss_function(y_true, y_pred):
    ...
    def dice_coeff(y_true_score, y_pred_score, training_mask):
    ...
    classification_loss = 0.01 * dice_coeff(y_true[:, :, :, -1:], y_pred[:, :, :, -1:], training_mask)
...
class TrainingSequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        images, score_maps, geo_maps, training_masks = load_augmentation_images(filenames, 512)
        if len(images) == 0:
...
class ValidationSequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        filenames = self.filenames[self.index[idx * self.batch_size: (idx + 1) * self.batch_size]]
        images, score_maps, geo_maps, training_masks = load_non_augmentation_images(filenames, 512)
        if len(images) == 0:
            return self.__getitem__((idx + 1) % self.__len__())
        return tf.convert_to_tensor(images), tf.convert_to_tensor(
            np.concatenate((geo_maps, score_maps, training_masks), axis=-1))
